...
@Directive({
  selector: '[appMediaElement]'
})
export class MediaElementDirective {
  @Input() config: any = {}
  @Input() stream: any = null
  ...
  convert() {
      let mediaElement = this.el.nativeElement.children[0]
      if(this.stream !== null){
    ...
    //   let updatedEl = this.getHTMLMediaElement(mediaElement, config)
    //   this.renderer.appendChild(this.el.nativeElement, updatedEl)
      setTimeout(()=>{
        // this.videoElement.play()
      },100)
  }
  getHTMLMediaElement(mediaElement, config) {
      const parent = this
      config = config || {};
      if(this.self) mediaElement.id= 'selfVideo'  
      ...
      parent.videoElement = mediaElement
      ...
      mediaElementContainer['toggle'] = function(clasName) {
          ...
          if (clasName == 'mute-audio' && muteAudio) muteAudio.onclick(this);
          if (clasName == 'mute-video' && muteVideo) muteVideo.onclick(this);

          if (clasName == 'record-audio' && recordAudio) recordAudio.onclick(this);
          if (clasName == 'record-video' && recordVideo) recordVideo.onclick(this);

          if (clasName == 'stop' && stop) stop.onclick(this);
  ...
  getAudioElement(mediaElement, config) {
      config = config || {};
